...
`init` takes only the **tier** and **type** from a template and records its name in the manifest; the resources it writes are the tier-derived ones in [Resource Auto-Generation](#resource-auto-generation) above, not the ones described in the table. So `--template critical-api` yields a `critical`-tier manifest with a 99.95% availability SLO, not the 99.9% the table names.
